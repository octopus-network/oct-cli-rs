        ...
        if self.confirm.eq("y") || self.confirm.eq("Y") {
            for account in account_list {
                ...
                clean_contract.deploy(&account).await.map_err(|error| {
                    color_eyre::Report::msg(format!("Failed to deploy clean state contract for account({}), error: {}", account.account_id, error))
                })?;

                clean_contract.clean_up_all(&account).await.map_err(|error| {
